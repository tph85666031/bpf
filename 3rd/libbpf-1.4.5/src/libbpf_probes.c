...
/* On Uos LINUX_VERSION_CODE doesn't correspond to info.release.
 * Instead, it is provided in /usr/include/linux/version.h. An example content of
 * Uos V20 looks like the below.
 *
 *   utsname::release   4.19.0-desktop-amd64
 *   utsname::version   #3100 SMP Thu Dec 17 19:05:15 CST 2020
 *
 * In the above, 4.19.260 is what kernel is actually expecting, while
 * uname() call will return 4.19.0 in info.release.
 */
static __u32 get_uos_kernel_version()
{
    if (faccessat(AT_FDCWD, "/etc/product-info", R_OK, AT_EACCESS) != 0)
    {
		return 0;
	}

    FILE* fp = fopen("/usr/include/linux/version.h", "r");
    if(fp == NULL)
    {
        return 0;
    }
    ...
    {
        char* p = strstr(buf, "LINUX_VERSION_CODE ");
        if(p != NULL && sscanf(buf, "%*s %*s %s", ver_str) == 1)
        {
            fclose(fp);
            return atoi(ver_str);
        }
...
__u32 get_kernel_version(void)
{
    ...
    version = get_uos_kernel_version();
	if (version != 0)
		return version;
